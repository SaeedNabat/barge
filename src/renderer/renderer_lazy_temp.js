...
// Lazy load Monaco editor function
...
async function loadMonacoEditor() {
	...
	if (editorContainer) {
		editorContainer.innerHTML = '<div style="display: flex; align-items: center; justify-content: center; height: 100%; color: #666;">Loading editor...</div>';
	}
	...
	try {
		...
		monacoLoading = false;
		if (editorContainer) {
			editorContainer.innerHTML = '<div style="display: flex; align-items: center; justify-content: center; height: 100%; color: #dc2626;">Failed to load editor</div>';
		}
	}
}

// Initialize Monaco editor after it's loaded
async function initializeMonacoEditor() {
	...
	monacoRef.editor.defineTheme("barge-light", {
		...
		colors: {
			...
			"scrollbarSlider.hoverBackground": "#64748b888",
	...
	monacoRef.editor.defineTheme("barge-dark", {
		...
		colors: {
			...
			"scrollbarSlider.background": "#64748b888",
...
async function createUntitled() {
	// Ensure Monaco is loaded before creating untitled file
	if (!monacoLoaded) {
		...
		async function ensureTerminal() {
			...
			if (!termInstance) {
				...
				setTimeout(() => {
					...
					if (window.bridge.terminal.write && termId) {
...
document.addEventListener('DOMContentLoaded', () => {
	...
	window.initializeTerminal = async () => {
		...
		// Ensure Monaco is loaded before activating tab
		if (!monacoLoaded) {
			await loadMonacoEditor();
			if (!monacoLoaded) {
				console.error("Failed to load Monaco editor");
			...
			// Update tab history
			try {
				// Remove path from history if it exists
				const idx = tabHistory.indexOf(path);
				if (idx > -1) tabHistory.splice(idx, 1);
				// Add to front of history
				tabHistory.unshift(path);
				// Keep history reasonable size (last 20 tabs)
		...
		// Expose markDirty globally for access from outside Monaco scope
		...
		try {
			const histIdx = tabHistory.indexOf(filePath);
			...
			if (tabIndex > 0) {
				...
				// If closed tab was first, activate the new first tab
				nextTabPath = openTabs[0].path;
			}
			if (nextTabPath) activateTab(nextTabPath);
			if (!nextTabPath && openTabs.length > 0) {
				nextTabPath = openTabs[0].path;
			}
			if (nextTabPath) activateTab(nextTabPath);
				}
		// Expose closeTab globally for access from outside Monaco scope
		...
		window.addEventListener('barge:pending-open', () => {
			if (window.__PENDING_OPEN__) {
				const p = window.__PENDING_OPEN__;
				// Only open file if Monaco is ready
				if (monacoRef) {
					openFileInTab(p.filePath, p.content);
					window.__PENDING_OPEN__ = null;
				} else {
					// Monaco still not ready, keep it pending
					console.log('Monaco not ready, keeping file pending:', p.filePath);
					// Don't clear the pending file, keep it for when Monaco is ready
				}
			}
		});

		// Add Monaco ready event listener
		...
		// Lazy load Monaco editor - only load when needed
		// Monaco will be loaded when first file is opened or tab is activated
		console.log("Monaco editor will be loaded on demand");
...
function forceClosePath(targetPath) {
	...
	try {
		...
		// Remove from tab history
		try {
			const histIdx = tabHistory.indexOf(targetPath);
			if (histIdx > -1) tabHistory.splice(histIdx, 1);
		} catch {}
		// Find the tab to the left of the closed tab
		let nextTabPath = null;
